        ...
        <div style={{ padding: '14px 14px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {activeEntry && (
            <div>
              ...
              <button className="btn btn-red no-drag" style={{ width: '100%', marginBottom: 4 }} onClick={onEndTask}>
                ⏹  End Current Task
              </button>
            </div>
          )}

          <div className="label-xs">{activeEntry ? 'LOG NEW TASK' : 'WHAT ARE YOU WORKING ON?'}</div>
          <input
            ref={descRef}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 10, color: 'var(--t3)' }}>{activeEntry ? 'switched at' : 'started at'}</span>
            <div style={{ flex: 1 }} />
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-primary no-drag" style={{ flex: 1 }} onClick={onLog}>
              {activeEntry ? 'Log New Task' : 'Start Tracking'}
            </button>
